...
module Problem42 =
    ...
    let filePath = "../../../../SolutionFS/Data/p042_words.txt"

    let triangleNumberSequence = 1 |> Seq.unfold (fun n -> Some((n * (n + 1)) / 2, n + 1))

    let solution() =
        
        let wordValues = filePath |> System.IO.File.ReadAllLines |> Array.map (SolutionCS.Utility.Miscellaneous.ConvertWordToNumber)
